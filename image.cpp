...
if(!f){ dprintf("Error: could not open file: %s", filename); exit(1);}
...
for(i=0; i<nb; i++){
  ...
  myBi = new SA<int>(3);
  for(i=0; i<3; i++){ (*myBi)[i]=i; } //set default band visualization to 1,2,3;
}
...
void myImg::scaleband( SA<float> * buf){ //this scaling needs to be fixed. zeroing data isn't quite OK.
float MAX = buf->max();
float MIN = buf->min();
printf(" MIN %e MAX %e \n", MIN, MAX);
int i=0; float dat;
for(i=0; i< buf->size(); i++){
  ...
  dat = buf->at(i);
if( isnan(dat) || isinf( dat)){ buf->at(i) = FLT_EPSILON; }
}
MAX = buf->max(); MIN = buf->min();
}
void myImg::printAsciiCSV(char * ASCIIFILE){
FILE * afile = fopen(ASCIIFILE, "wb");
if(!afile){ dprintf("Error: could not open file: %s\n", ASCIIFILE); exit(1);}
int kk, i;
for(kk=0; kk<NRow*NCol; kk++){
for(i=0; i<NBand; i++){ fprintf(afile, "%7.7e,", (float_buffers->at(i))->at(kk)); }
fprintf(afile, "\n");
}fclose(afile);
}